...
public class TransListAdapterSend extends BaseAdapter {
    ...
    @Deprecated
    public TransListAdapterSend(final Context context, @Nonnull final AbstractWallet walletPocket) {
        this.context = context;
        inflater = LayoutInflater.from(context);
        this.walletPocket = walletPocket;
        res = context.getResources();
        colorSignificant = this.res.getColor(R.color.app_green);
        colorLessSignificant = this.res.getColor(R.color.gray_54_sec_text_icons);
        colorInsignificant = this.res.getColor(R.color.gray_26_hint_text);
        colorError = this.res.getColor(R.color.fg_error);
        minedTitle = this.res.getString(R.string.wallet_transactions_coinbase);
        fontIconMined = this.res.getString(R.string.font_icon_mining);
        sentToTitle = this.res.getString(R.string.sent);
        receivedWithTitle = this.res.getString(R.string.received);
        receivedFromTitle = this.res.getString(R.string.received);
    ...
    public void replace(Collection<AbstractTransaction> transactions) {
    ...
    @Override
    public AbstractTransaction getItem(int position) {
        if (position == this.transactions.size()) {
            return null;
        }
        return (AbstractTransaction) this.transactions.get(position);
    }
    ...
    public long getItemId(int position) {
        ...
        return WalletUtils.longHash(((AbstractTransaction) this.transactions.get(position)).getHashBytes());
    }
    ...
    public View getView(int position, View row, ViewGroup parent) {
        ...
        if (type == 0) {
            if (row == null) {
                row = this.inflater.inflate(R.layout.transaction_row, null);
            }
...
//                rowValue.setBackgroundResource(R.drawable.rv_receive_bg);
                ...
                rootlayout.setLayoutParams(new android.widget.LinearLayout.LayoutParams(0, 0));
                v_down.setVisibility(View.GONE);
            }
        ...
        if (tx.getDepthInBlocks() < 4) {
            ...
            switch (tx.getDepthInBlocks()) {
                case 0:     // No confirmations
                    ...
                    rowConfirmationsFontIcon.setColorFilter(colorInsignificant);
                    break;
...
//        }
    ...
    private String resolveLabel(AbstractAddress address) {
        String cachedLabel = (String) this.labelCache.get(address);
        if (cachedLabel == null) {
            String label = AddressBookProvider.resolveLabel(this.context, address);
            if (label != null) {
                this.labelCache.put(address, label);
                return label;
            }
            this.labelCache.put(address, (String) CACHE_NULL_MARKER);
            return label;
        }
